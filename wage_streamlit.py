...
st.header("2019年：一人当たり賃金ヒートマップ")

jp_lat_lon=pd.read_csv("./pref_lat_lon.csv")
jp_lat_lon=jp_lat_lon.rename(columns={"pref_name": "都道府県名"})

show_df=st.checkbox("show lat_lon")
if show_df==True:
    st.dataframe(jp_lat_lon)

df_pref_map=df_pref_ind[(df_pref_ind["年齢"]=="年齢計")&(df_pref_ind["集計年"]==2019)]
df_pref_map=pd.merge(df_pref_map, jp_lat_lon, on="都道府県名")
df_pref_map["一人当たり賃金正規化"]=((df_pref_map["一人当たり賃金（万円）"]-df_pref_map["一人当たり賃金（万円）"].min())/(df_pref_map["一人当たり賃金（万円）"].max()-df_pref_map["一人当たり賃金（万円）"].min()))
...
show_df=st.checkbox("show DateFrame")
if show_df==True:
    st.write(df_pref_map)

st.header("集計年別一人当たり賃金推移")
...
wage_list=df_jp_category.columns[4:7]
...
st.text("出展：RESAS_地域経済分析システム")
st.text("本結果はRESAS_地域経済分析システムを加工して作成")